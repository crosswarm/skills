...
用户提供账号后，AI 助手在后台执行 `--login`，登录成功后立即重跑刚才失败的请求。
...
> 最终回复由 **AI 助手**基于所有检索证据重新撰写。

**从返回值中提取的上下文数据**（仅供 AI 助手推理使用）:

| 字段 | 用途 |
|------|------|
| `ai_analysis.recommended_team` / `recommended_role` | 路由建议（展示给用户） |
| `ai_analysis.functionality_impact` | 功能影响描述（作为回复参考） |
| `ai_analysis.solution_suggestion` | 解决方向提示（作为回复参考） |
...
| `kb_sources` / `kb_refs` | KB 引用列表（展示标题；内容供参考） |
...
基于以下所有上下文，由 **AI 助手**撰写最终回复（不得复制服务端 reply_content）：
...
**触发条件**: 当前对话中已有一个最近的 issue_key，且用户对 AI 助手上一轮给出的回复进行了手工修改或补充关键词。
...
AI 助手基于以下材料重写最终回复（格式同模式 A 阶段三，标题改为「方案完善」）：
...
4. **用户友好引导** — 登录、额度提示、错误提示全部用自然语言；**禁止让用户自己跑任何命令行脚本**，AI 助手代劳所有脚本调用
5. **429 必须转化为登录引导** — 永远不直接向用户输出「HTTP 429」或「额度超限」原始错误，统一转化为友好的登录引导
6. **refined_solution 不直接输出** — 与 reply_content 一样，服务端生成的方案文本只作为 AI 助手推理材料，最终回复由 AI 助手重新撰写
