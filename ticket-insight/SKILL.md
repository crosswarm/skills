...
> **主题聚合六铁律（所有用本 skill 的 agent 必须遵守，别绕过）**
...
> 6. **主题数量控制**：业务主题总数**目标 ~20，上限 30**（门禁③拦 >30），非特殊情况不得超。太碎 → 把最小的长尾主题（`--batches` 的 `tail`）用 edits `merge`+`rename` **合并成 1 个业务风格命名的聚合主题（禁"其他/杂项"）**，再重跑。此举同时让四维度覆盖率更易达 75%。
...
python3 scripts/ti_themes.py --workdir <WD> --gate                # 门禁①覆盖率≤3% + ②无过度聚合 + ③主题数≤30
```
...
4. **重跑聚合** → 看三个门禁：
   ...
   - **门禁③主题数量**：业务主题 ≤30（目标~20）。未过 → 把最小的长尾主题合并成 1 个业务命名聚合主题（edits merge+rename，禁"其他"）→ 重跑。
5. 循环 4，直到三个门禁都过（或用户显式豁免，豁免记入报告口径）。
...
**S3.3 一级主题命名（必做）**：你(Claude)编辑 `themes/<PROJ>/sub-themes.yaml` 的 `labels`——给**报告中出现的一级主题**（各维度 top_themes + all_tops）起**清晰易懂的业务风格显示名**（现有 id 如 `I-流程设计-分支条件` 太机械）。命名规则：**别太简洁看不明白，也别复杂不易懂**（如 `审批流分支条件配置`、`审批找人/选人规则`）。报告全用 label，防幻觉校验认 id∪label。
> ⚠️ **二级子主题下钻（`sub_themes`）2026-07-07 起暂禁**——`ti_analyze` 不再产出二级分布，报告不渲二级表。代码保留待启用；本阶段只做一级 `labels`。

**S3.5 智能总结（必做）**：分析完成后，你(Claude)按 `references/insights-template.md` 协议撰写 `<WD>/data/insights.md`（主题名用 labels 友好显示名，与报告一致）。核心要求：
- **每业务维度覆盖 ≥75%**：先列该维度 top 主题（analysis.json `top_themes` 已按覆盖贪心选到约75%），再把余下长尾并成**一条 Agent 业务风格命名**的"长尾合并"项（用 `dimensions[].tail` 数字，**禁命名"其他/杂项"**）。`ti_report` 会软校验覆盖率，<75% 报警。
- **措施更智能**：按杠杆优先——根因消除/产品化/默认值（一劳永逸）> 跨主题跨维度一个动作多压降（一劳多得）> 规模化自助 > 一次性人工；每维度至少 1 条结构性措施，措施末尾标注杠杆类型。
- 只引用 analysis.json 真实主题名与数字（ti_report 校验，未知名→阻断）；预期效果=基数×参考压降率并**标注估算**；连知识库则结合背景细化并标来源；简体中文。
...
- 报告结构：概况→KPI→月度→主题分布→**智能总结与改进建议**→四维度专项（含各维度长尾合并行，无每维度客户表）→**重点客户 Top10（独立成章）**→附口径（含知识库来源行）
